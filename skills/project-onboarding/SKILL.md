...
description: Use this skill whenever a user wants to adopt the scicomp-research-skills framework on an EXISTING project (paper, software library, analysis repo, rebuttal workspace) rather than starting from scratch. Make sure to load this whenever the user says "I have an existing project; let's start using these skills on it" or equivalent, EVEN IF THE USER DOES NOT MENTION IT EXPLICITLY. The skill covers two top-level scenarios -- (1) existing project with NO prior agentic instructions (no AGENTS.md / CLAUDE.md / .cursorrules / etc.); (2) existing project WITH prior agentic instructions (one or more client-specific files already present) -- and several sub-cases per scenario (empty-ish vs mature vs non-standard layout for Scenario 1; one vs many agent files vs substantive existing content vs conflicting conventions for Scenario 2). Provides an inventory-first migration workflow that preserves existing project content, surfaces conflicts to the user explicitly, and never silently overwrites. Companion to the "Starting a new project" docs in root AGENTS.md Section 11 + README.md (which assume a from-scratch start). Composes with `human-facing-doc-authoring` (the migrated AGENTS.md + PLAN.md must follow its conventions) and `agent-resource-discipline` (first-action protocol applies to onboarding sessions just like any other).
license: MIT
metadata:
